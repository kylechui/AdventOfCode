...
class FileSystem {
   public:
    ...
    [[nodiscard]] auto getDirectories() const -> vector<shared_ptr<Directory>>;
    [[nodiscard]] auto getRoot() const -> shared_ptr<Directory>;
...
[[nodiscard]] auto FileSystem::getDirectories() const
    -> vector<shared_ptr<Directory>> {
    auto seen = unordered_set<shared_ptr<Directory>>{};
    auto dfs = [&](auto&& dfs, const auto& dir) -> void {
        if (seen.contains(dir)) return;
...
[[nodiscard]] auto FileSystem::getRoot() const -> shared_ptr<Directory> {
    return root;
}
...
auto main() -> int {
    ...
    const auto TOTAL_SIZE = ll{70'000'000};
    const auto MIN_UNUSED_SIZE = ll{30'000'000};
    auto smallestSize = LLONG_MAX;
    auto fs = FileSystem(lines);
    for (const auto& directory : fs.getDirectories()) {
        if (TOTAL_SIZE - fs.getRoot()->getSize() + directory->getSize()
            >= MIN_UNUSED_SIZE) {
            smallestSize = min(smallestSize, directory->getSize());
        }
    }
